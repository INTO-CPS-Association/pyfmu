...
set<string> examples = {"Adder",     "ConstantSignalGenerator", "SineGenerator",
                        "LoggerFMU", "BicycleKinematic",        "FmiTypes"};
...
ExampleArchive::ExampleArchive(std::string exampleName)
  ...
  int retValue =
      std::system("python -c \"import sys; found = 0 if (sys.version_info >= "
                  "(3,0)) else -1; sys.exit(found)\"");
  ...
  string exportCommand =
      format("{} export --project {} --out {}", exporter_script_name,
             examplePath.string(), exportPath.string());

  info("exporting example project {} using command: {}", exampleName,
       exportCommand);
  ...
  if (ret != 0) {
    throw runtime_error(
        format("Export of example project failed, command returned {}", ret));
  }
}

fs::path ExampleArchive::getRoot() { return this->td.root / exampleName; }

fs::path ExampleArchive::getResources() { return getRoot() / "resources"; }

string ExampleArchive::getResourcesURI() {

  return pyfmu::getFileUriFromPath(getResources());
}