...
public:
  ...
  void Free(void *ptr) CPPFMU_NOEXCEPT { m_free(ptr); }
...
public:
  ...
  Logger(FMIComponentEnvironment component, String instanceName,
         FMICallbackFunctions callbackFunctions,
         std::shared_ptr<Settings> settings)
      : m_component{component}, m_instanceName(std::move(instanceName)),
        m_fmiLogger{callbackFunctions.logger}, m_settings{settings} {}

  // Logs a message.
  template <typename... Args>
  void Log(FMIStatus status, FMIString category, FMIString message,
           Args &&... args) CPPFMU_NOEXCEPT {
    if (m_settings->loggedCategories.empty() ||
        detail::CanFind(m_settings->loggedCategories, category)) {
      m_fmiLogger(m_component, m_instanceName.c_str(), status, category,
   ...
   */
  template <typename... Args>
  void DebugLog(FMIStatus status, FMIString category, FMIString message,
                Args &&... args) CPPFMU_NOEXCEPT {
